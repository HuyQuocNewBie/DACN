          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50 transition-colors duration-300 dark:divide-slate-800/50">
              {currentUsers.map((user) => (
                <tr
                  key={user.id}
                  className="group transition-all duration-300 hover:bg-slate-50/80 dark:hover:bg-slate-800/50"
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs font-medium text-slate-400 transition-colors duration-300 dark:text-slate-500">
                          {user.email}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-5 text-center">
                    <span
                      className={`rounded-xl px-3 py-1.5 text-[10px] font-black tracking-widest uppercase transition-colors duration-300 ${
                        user.role === 'admin'
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className="flex justify-center">
                      <span
                        className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-[10px] font-black tracking-widest uppercase transition-colors duration-300 ${
                          user.status === 'active'
                  ...
                  <td className="px-8 py-5 text-right">
                    <button
                      ...
                      className={`rounded-xl px-5 py-2 text-[10px] font-black tracking-widest uppercase shadow-sm transition-all active:scale-95 ${
                        user.status === 'active'
                          ? 'bg-slate-900 text-white hover:bg-rose-600 dark:bg-slate-700 dark:hover:bg-rose-600'
        ...
        {filtered.length > itemsPerPage && (
          <div className="mt-4 flex items-center justify-center gap-2">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-100 bg-white font-bold transition-all hover:bg-slate-50 disabled:opacity-20 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
            >
              ←
            </button>
            {[...Array(totalPages)].map((_, i) => (
              <button
                ...
                className={`h-10 w-10 rounded-xl text-xs font-black transition-all ${
                  currentPage === i + 1
                    ? 'bg-slate-900 text-white shadow-lg dark:bg-primary dark:text-white'
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <div className="group flex items-center justify-between rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900">
          <div>
            ...
            <h4 className="mt-1 text-3xl font-black text-slate-900 transition-colors duration-300 dark:text-white">
              {stats.activeRate}%
            </h4>
          </div>
